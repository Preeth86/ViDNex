...
def node_embedding_and_mapping(servers, vnr):
    ...
    for vm_index, vm_cpu in enumerate(vnr['vm_cpu_cores'], start=1):
        ...
        for server_id in servers:
            # ADDITIONAL CONSTRAINT: Ensure no two VMs from the same VNR are mapped to the same server
            if server_id in vnr_to_server_assignments.get(vnr['vnr_id'], []):
                continue  # Skip this server if another VM from the same VNR is already assigned

            if host_flag[server_id] and servers[server_id]['cpu'] >= vm_cpu:
                vm_to_server_assignments[f"VM{vm_index}"] = server_id
                vnr_to_server_assignments.setdefault(vnr['vnr_id'], []).append(server_id)
                host_flag[server_id] = False
                servers[server_id]['cpu'] -= vm_cpu
                servers[server_id]['vms'].append({'vnr_id': vnr['vnr_id'], 'vm_index': vm_index, 'cpu': vm_cpu})
                custom_print(f" VM{vm_index} mapped to {server_id} with {vm_cpu} CPU.")
                assigned = True
                break

        if not assigned:
            custom_print(f" No suitable server found for VM{vm_index}. Mapping failed.")
            return {}, {}, servers  # Fail the mapping if any VM can't be placed
