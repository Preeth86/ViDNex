...
def load_sn_graph(sn_json):
    ...
    # Add node degree as an attribute for servers
    for node in graph.nodes:
...
def calculate_ahp_weights(criteria_scores):
    # Use only meaningful server-specific metrics
    criteria = ['P_k_U_cpu', 'load_balance', 'degree', 'farness', 'eigenvector']

    # Expert-defined pairwise comparison matrix reflecting the new priorities
    matrix = np.array([
        [1, 1, 3, 4, 5],       # Energy (P_k_U_cpu)
        [1, 1, 3, 4, 5],       # Load balance
        [1/3, 1/3, 1, 2, 3],   # Degree
        [1/4, 1/4, 1/2, 1, 2], # Farness
        [1/5, 1/5, 1/3, 1/2, 1] # Eigenvector
    ])
    ...
    #     custom_print(f"{k}: {v:.4f}")
    return weight_dict

def calculate_vikor_rankings(criteria_scores, weights):
    criteria = list(weights.keys())

    # Normalize scores
    for c in criteria:
        values = [score[c] for score in criteria_scores]
        min_val, max_val = min(values), max(values)
        for score in criteria_scores:
            score[c] = (score[c] - min_val) / (max_val - min_val + 1e-6)

    f_star = {c: min(score[c] for score in criteria_scores) for c in criteria}
    f_minus = {c: max(score[c] for score in criteria_scores) for c in criteria}
    ...
    for score in criteria_scores:
        ...
        R_i = -float('inf')
        for c in criteria:
            norm = (score[c] - f_star[c]) / (f_minus[c] - f_star[c] + 1e-6)
            weighted = weights[c] * norm
            S_i += weighted
            R_i = max(R_i, weighted)

        # Penalize idle server usage
        is_idle = int(score.get('is_idle', 1))  # 1 if idle, 0 if already used
        penalty = 0.3 * is_idle
    ...
    min_S_i = min(r['S_i'] for r in rankings)
    max_S_i = max(r['S_i'] for r in rankings)
    min_R_i = min(r['R_i'] for r in rankings)
    max_R_i = max(r['R_i'] for r in rankings)

    for ranking in rankings:
        S_i = ranking['S_i']
        R_i = ranking['R_i']
        Q_i = 0.5 * (S_i - min_S_i) / (max_S_i - min_S_i + 1e-6) + \
              0.5 * (R_i - min_R_i) / (max_R_i - min_R_i + 1e-6)
        ranking['Q_i'] = Q_i + ranking['penalty']

    rankings.sort(key=lambda x: x['Q_i'])

    # Print sorted ranks before returning
    # custom_print("\nSorted Server Rankings (by Q_i):")
    # for rank in rankings:
    #     custom_print(f"Server ID: {rank['server_id']}, Q_i: {rank['Q_i']:.4f}")

    return [r['server_id'] for r in rankings]

def node_embedding_and_mapping(servers, vnr, graph, global_used_servers):
    custom_print(f"\nNode Embedding and Mapping of VMs for VNR ID: {vnr['vnr_id']}")
    P_idle, P_full = 150, 300
    ...
    farness = {node: 1 / val if val != 0 else 0 for node, val in closeness.items()}
    eigenvector = nx.eigenvector_centrality(graph, max_iter=1000)

    for vm_index, vm_cpu in enumerate(vnr['vm_cpu_cores'], start=1):
        custom_print(f"\n VM{vm_index} requires {vm_cpu} CPU cores.")
        criteria_scores = []

        for server_id, server_info in servers.items():
            if server_info['cpu'] >= vm_cpu and all(v['vnr_id'] != vnr['vnr_id'] for v in server_info['vms']):
                ...
                load_balance = 1 - ((server_info['original_cpu'] - server_info['cpu']) / server_info['original_cpu'])
                degree = graph.nodes[server_id]['degree']
                f = farness.get(server_id, 0)
                e = eigenvector.get(server_id, 0)
                is_idle = 1 if server_id not in global_used_servers else 0
        ...
        if not criteria_scores:
            custom_print(f"❌ No suitable server found for VM{vm_index}. Rolling back.")
            rollback_embedding(vm_to_server_assignments, [], servers, None)
            return {}, {}, servers

        ahp_weights = calculate_ahp_weights(criteria_scores)
        ranked_servers = calculate_vikor_rankings(criteria_scores, ahp_weights)

        best_server = None
        for server_id in ranked_servers:
            if server_id not in used_servers and \
               servers[server_id]['cpu'] >= vm_cpu and \
               all(v['vnr_id'] != vnr['vnr_id'] for v in servers[server_id]['vms']):
                best_server = server_id
                break

        if best_server is None:
            custom_print(f"❌ No ranked server suitable for VM{vm_index}. Rolling back.")
            rollback_embedding(vm_to_server_assignments, [], servers, None)
            return {}, {}, servers
        ...
        used_servers.add(best_server)
        global_used_servers.add(best_server)
        custom_print(f"✅ VM{vm_index} mapped to {best_server} with {vm_cpu} CPU.")
...
def main():
    ...
    for vnr in vnr_graph:
        ...
        vm_to_server_assignments, vnr_to_server_assignments, updated_servers = node_embedding_and_mapping(servers, vnr, graph, global_used_servers)
